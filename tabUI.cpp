	...
	if (_sv)
	{
		...
		for (int i = 0; i < 5; ++i)
		{
			...
			if (_saveSlot[i].isSaved)
			{
				...
				// ∑π∫ß
				D2DRENDERER->RenderText(_saveSlot[i].rc.left, 200, to_wstring(_saveSlot[i].level), 20, D2DRenderer::DefaultBrush::White, DWRITE_TEXT_ALIGNMENT_LEADING, L"∏º¿∫∞ÌµÒBold");
				...
				D2DRENDERER->RenderText(_saveSlot[i].rc.left + 560, _saveSlot[i].rc.top + 85, t, 20, D2DRenderer::DefaultBrush::White, DWRITE_TEXT_ALIGNMENT_LEADING, L"∏º¿∫∞ÌµÒBold");

				// Ω√∞£
				D2DRENDERER->RenderText(_saveSlot[i].rc.left + 1240, _saveSlot[i].rc.top + 15, to_wstring(_saveSlot[i].time), 20, D2DRenderer::DefaultBrush::White, DWRITE_TEXT_ALIGNMENT_LEADING, L"∏º¿∫∞ÌµÒBold");
			}
		}
	}
	if (_st)
...
void tabUI::inSave()
{
	...
	// TODO: ¡§∫∏ ∞°¡ÆøÕº≠ slotø° ºº∆√«œ±‚
	// TODO: «ÿ¥Á ¡§∫∏ πŸ≈¡¿∏∑Œ RECT º≥¡§...
}
void tabUI::inStat()
{
	_eq = false;
